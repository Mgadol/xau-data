...
def detect(df):
    ...
    for i in cand:
        ...
        # АНКР нь ҮРГЭЛЖ BOS бар. Урьд нь «давталт байвал давталтын
        # бар, үгүй бол displacement» гэж байсан нь ЗАНГА: `has_retest`
        # бол ИРЭЭДҮЙН хувьсагч (BOS-ийн дараах 8 барт үнэ буцсан уу).
        # Оролтын барыг түүгээр сонговол «буцаагүй» бүлэг нь ирээдүйг
        # мэдсэн сонголт болно. Тэр хольцоор 15 минут дээр 61.6% гарч
        # байсан ч, ижил эвентүүдийг BOS барт ороход 52.2% болно —
        # бүх «ирмэг» нь тэр сонголтоос гарч байжээ. Давталт хүлээх нь
        # ӨӨР стратеги тул `--entry retest`-ээр тусад нь хэмжинэ.
        ai = bi
...
def main():
    ...
    ap.add_argument('--entry', choices=('bos', 'retest'), default='bos',
                    help='bos = BOS барт ор (бүх эвент). '
                         'retest = давталт болохыг хүлээж ор (эвентийн '
                         '~20%). Хоёр нь ӨӨР стратеги — хольж БОЛОХГҮЙ.')
    a = ap.parse_args()
    ...
    e = detect(df)
    if a.entry == 'retest':
        # Давталт болохыг ХҮЛЭЭЭД орох — бүрэн өөр, илүү жижиг стратеги.
        e = e[e.has_retest].copy()
        e['anchor_i'] = e['retest_i']
    e = dedup(e)
    print(f'  оролт: {a.entry}')
    ...
    # ── ДАВТАЛТЫН ЗАНГА ──────────────────────────────────────────
    # `has_retest` нь BOS-ийн ДАРААХ 8 барт үнэ буцсан эсэх — өөрөөр
    # хэлбэл ИРЭЭДҮЙ. Эвентүүдийг түүгээр хувааж, «давталтгүй» бүлгийг
    # BOS барт орсон гэж тооцвол ирээдүйг мэдсэн сонголт болно:
    #
    #   15 минут, 3 ATR, багц ТОГТМОЛ:
    #     давталтгүй · анкр BOS        n=915   62.0%   p<0.0001
    #     давталттай · анкр ДАВТАЛТ    n=205   53.7%   p=0.29
    #     давталттай · анкр BOS        n=237   14.3%   ← утгагүй
    #     БҮГД       · анкр BOS       n=1152   52.2%   p=0.14
    #
    # 14.3% нь зангыг илчилнэ: «давталт болно» гэдгийг мэдсэн бол үнэ
    # буцахыг мэдсэн хэрэг. Тэр хоёр бүлгийг нийлүүлээд л жинхэнэ тоо
    # (52.2%) гарна. Тиймээс энд хуваалт хийхгүй — `--entry retest`
    # нь давталтыг ХҮЛЭЭДЭГ тусдаа стратеги болж хэмжигдэнэ.
